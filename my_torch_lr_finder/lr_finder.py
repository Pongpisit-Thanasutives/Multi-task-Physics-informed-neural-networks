...
class LRFinder(object):
    ...
    def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
        ...
        for i in range(accumulation_steps):
            ...
            if not self.closure and type(loss)==list: 
                loss = sum(loss)
            ...
            if IS_AMP_AVAILABLE and hasattr(self.optimizer, "_amp_stash"):
                ...
                if self.closure and type(loss)==list:
                    self.model, self.optimizer, loss = self.closure(self.model, self.optimizer, loss)
                else:
                    loss.backward()
    ...
    def plot(
        ...
        if suggest_lr and min_grad_idx:
            return lrs[min_grad_idx], ax
        else:
            return lrs[min_grad_idx], None
